function [u1, u2, p, res, res_u1, res_u2, res_p,  x, y, u_ex] = Stokes_solver(u1_b, u2_b, p_b, f1_b, f2_b, xy, mesh, dirtot, mu, c, tau, bu)
...
%mesh's data
    ...
    u1_ex = u1(1:Nodes - N_elem_b);
    u2_ex = u2(1:Nodes - N_elem_b);
    p_ex = p(1:Nodes - N_elem_b);
 ...
 %%% computing global matrices and rhs
    ...
    %residual error
    res_u1 = norm(u1_ex - u1);
    res_u2 = norm(u2_ex - u2);
    res_p = norm(p_ex - p);
    res = norm(A*u_ex - rhs);
end